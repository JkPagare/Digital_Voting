...
def vote(request):
    if request.method == "POST":
        ...
        if vote.v.has_voted == 'no':
            ...
            # Open webcam
            cam = cv2.VideoCapture(0)  # Use 0 for default webcam, you can change it to 1, 2, etc. for other webcams
             
            rec = cv2.face.LBPHFaceRecognizer_create()
            rec.read(os.path.join(BASE_DIR, "TrainingImageLabel", "Trainner.yml"))
            faceDetect = cv2.CascadeClassifier(os.path.join(BASE_DIR, "haarcascade_frontalface_default.xml"))
